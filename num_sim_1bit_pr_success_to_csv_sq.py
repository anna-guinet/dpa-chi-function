...
def create_fig(K, kappa, frames):
	...
	plt.title(r'DoM with squared scalar product | K=%s & kappa=%s' %(K, kappa))
	plt.xlabel('Weight w')
...
def write_csv(n, i, K, kappa, num_sim, frames):
	...
	num_rank = 1
	for df in frames:
		file_name = r'./csv/1bit_%srow_sq_num-sim=%s_K=%s_kappa=%s_i=%s_rank%s.csv' % (n, num_sim, K, kappa, i, num_rank)
...
def sim_1bit_to_csv(n, i, K, kappa, num_sim):
	...
	# plt.savefig('./plot/1bit_%srow_sq_num-sim=%s_K=%s_kappa=%s_i=%s.png' % (n, num_sim, K, kappa, i))
	# plt.close(fig)
	# plt.show()
