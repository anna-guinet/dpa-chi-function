...
def create_fig(K, kappa, frames):
	...
	plt.title(r'Combined DoM with absolute scalar product | K=%s & kappa=%s' %(K, kappa))
	plt.xlabel('Weight w')
...
def write_csv(K, kappa, num_sim, frames):
	...
	num_rank = 1
	for df in frames:
		file_name = r'./csv/1x5bits_abs_num-sim=%s_K=%s_kappa=%s_rank%s.csv' % (num_sim, K, kappa, num_rank)
		df.to_csv(file_name, encoding='utf-8', index=False)
		num_rank += 1

def sim_1x5bits(K, kappa, num_sim):
	"""
	Compute scalar products for several simulations. 
	...
	for j in range(num_sim):
		...
		# ------------------------------------------------------------------------------------------- #
		# Find intersections with solution function
...
def sim_1x5bits_to_csv(K, kappa, num_sim):
	...
	# plt.savefig('./plot/1x5bits_abs_num-sim=%s_K=%s_kappa=%s.png' % (num_sim, K, kappa))
	# plt.close(fig)
	# plt.show()
